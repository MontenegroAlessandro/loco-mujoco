...
#     fwd_cone_deg: float = 70.0,   # 前向扇形角，越小越“只往前”
# ) -> List[Action]:
#     rng = np.random.default_rng(seed)
#     uniq: Dict[Tuple[int, int, int], Action] = {}
#
#     max_tries = n * 200  # 去重/约束导致采样被拒绝时防死循环
#     tries = 0
#
#     cone = math.radians(fwd_cone_deg) * 0.5
#
#     while len(uniq) < n and tries < max_tries:
#         tries += 1
#
#         # sample in polar (local frame)
#         r = rng.uniform(cons.r_min, cons.r_max)
#         alpha = rng.uniform(-cone, +cone)
#
#         dx = r * math.cos(alpha)
#         dy = abs(r * math.sin(alpha))  # store magnitude only
#
#         # enforce lateral separation
#         if dy < cons.feet_distance:
#             continue
#
#         # optional backward steps
#         if allow_backward and (rng.random() < 0.25):
#             dx = -dx
#
#         # sample yaw change + (optional) quantize to yaw bins
#         dyaw = rng.uniform(-cons.dyaw_max, cons.dyaw_max)
#         dyaw = round(wrap_to_pi(dyaw) / cfg.theta_res) * cfg.theta_res
#
#         a = Action(dx=float(dx), dy=float(dy), dyaw=float(dyaw))
#
#         # de-dup (quantize by lattice resolution)
#         key = (
#             int(round(a.dx / cfg.xy_res)),
#             int(round(a.dy / cfg.xy_res)),
#             int(round(wrap_to_pi(a.dyaw) / cfg.theta_res)),
#         )
#         uniq[key] = a
...
#     ):
#         """
#         obstacle_side: square side length (0.3)
#         foot: rectangle approximation for the foot sole
#         margin: safety margin added to obstacles and foot half-sizes
#         grid_cell_size: broadphase hash cell size
#         """
#         self.obs_centers = obstacle_centers
#         self.obs_half = 0.5 * float(obstacle_side)
...
#
#         # Broadphase radius: obstacle half-diagonal + foot half-diagonal + margin
#         obs_r = math.sqrt(2.0) * (self.obs_half + self.margin)
#         foot_r = math.sqrt((self.foot_hx + self.margin) ** 2 + (self.foot_hy + self.margin) ** 2)
#         self.query_radius = obs_r + foot_r
#
#         self.grid = GridHash(self.obs_centers, cell_size=grid_cell_size)
#
#     def is_footstep_valid(self, s: "State") -> bool:
#         """
#         Returns True if foot placement at stance s is collision-free w.r.t obstacles.
#         """
#         # inflate both foot and obstacle by margin (conservative)
#         fhx = self.foot_hx + self.margin
#         fhy = self.foot_hy + self.margin
#         oh  = self.obs_half + self.margin
#
#         # Broadphase: only check nearby obstacles
#         candidates = self.grid.query(s.x, s.y, self.query_radius)
#
#         for (ox, oy) in candidates:
#             if sat_oriented_rect_vs_aabb_square(
#                 foot_cx=s.x, foot_cy=s.y, foot_yaw=s.yaw, foot_hx=fhx, foot_hy=fhy,
#                 obs_cx=ox, obs_cy=oy, obs_half=oh,
#             ):
#                 return False
#         return True
#
...
#         """
#         Extra collision checks for the STEP transition u -> v:
#           1) corridor capsule between u.xy and v.xy (leg/hip corridor)
#           2) swing-foot swept-volume approximation (also u.xy -> v.xy capsule, smaller radius)
#         """
#         # inflate obstacles by margin too
#         oh = self.obs_half + self.margin
#
#         # broadphase: query around the segment midpoint
#         mx = 0.5 * (u.x + v.x)
#         my = 0.5 * (u.y + v.y)
#         seg_len = hypot2(v.x - u.x, v.y - u.y)
#
#         # worst-case inflate for querying candidates
#         inflate_max = max(self.corridor_radius, self.swing_radius) + self.margin
#         query_r = 0.5 * seg_len + math.sqrt(2.0) * (oh + inflate_max)
#
#         candidates = self.grid.query(mx, my, query_r)
#
#         for (ox, oy) in candidates:
#             # (A) corridor: wider
#             if _segment_hits_square_inflated(
#                     u.x, u.y, v.x, v.y, ox, oy, self.obs_half,
#                     inflate=self.corridor_radius + self.margin
#             ):
#                 return False
#
#             # (B) swing swept approximation: narrower
#             if _segment_hits_square_inflated(
#                     u.x, u.y, v.x, v.y, ox, oy, self.obs_half,
#                     inflate=self.swing_radius + self.margin
#             ):
#                 return False
#
#         return True
