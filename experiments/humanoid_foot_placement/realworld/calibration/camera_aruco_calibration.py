...
# Create Board #
CHARUCO_BOARD = aruco.CharucoBoard((5, 3),
    squareLength=0.053,
    markerLength=0.0397,
...
class CharucoDetector:
    ...
    def augment_image(self, cam_id, image, visualize=False, visual_type=["markers", "axes"]):
        ...
        if readings is None:
            if visualize:
                cv2.imshow("Charuco board no readings: {0}".format(cam_id), image)
                cv2.waitKey(20)
            return image
...
if __name__ == "__main__":
    ...
    rclpy.init()
    node = rclpy.create_node("camera_aruco_calibration_node")
    ...
    cam_info_sub = node.create_subscription(
        msg_type=CameraInfo,
        topic="/camera/camera/color/camera_info",
        callback=cam_info_callback,
        qos_profile=10,
    )

    while rclpy.ok():
        rclpy.spin_once(node)
        if image is not None and len(intrinsics_dict) > 0:
            break
    detector = CharucoDetector(
        intrinsics_dict={"test_cam": intrinsics_dict},
        inlier_error_threshold=3.0,
        reprojection_error_threshold=3.0,
        num_img_threshold=1,
    ...
    readings = detector.process_image(image)
    target2cam_results = detector.calculate_target_to_cam([readings], train=True)

    foot_to_cam_pos = np.array([ 0.1935288, -0.01939153, 0.81349464]) 
    foot_to_cam_rot = np.array([[ 0.13349626,  0.98737645, -0.08524376], 
                                [-0.99078889,  0.13099484, -0.03431789], 
                                [-0.02271818,  0.08903988,  0.99576894]])
    

    if target2cam_results is not None:
        R_target2cam, t_target2cam, successes = target2cam_results
        # print("R_target2cam: ", R_target2cam)
        print("t_target2cam: ", t_target2cam)

        t_target2cam[0][2] *= -1  # Flip Z axis

        foot_to_target = foot_to_cam_rot.T @ ( t_target2cam[0]) + foot_to_cam_pos
        foot_to_target_rot = foot_to_cam_rot.T @ R_target2cam[0]

        print("foot_to_target_pos: ", foot_to_target)
        # print("foot_to_target_rot: ", foot_to_target_rot)
