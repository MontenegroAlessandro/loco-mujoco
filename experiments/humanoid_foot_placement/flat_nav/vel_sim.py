...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="vel_conf.yaml",
    ...
    # --- Load Policy ---
    policy = LMJPolicy(policy_path=agent_path) # Removed control_func_path
    ...
    cmd = np.zeros(6, dtype=np.float32) # Command vector: [vel_x, vel_y, vel_yaw, height, cos(gait), sin(gait)]
    counter = 0
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running() and (time.time() - start_time) < simulation_duration:
            ...
            if counter % control_decimation == 0:
                ...
                gait_process = (counter * simulation_dt * cmd_params["gait_frequency"]) % 1.0
                cos_val = np.cos(2 * np.pi * gait_process)
                sin_val = np.sin(2 * np.pi * gait_process)
                cmd = np.array([
                    cmd_params["vel_x"],
                    cmd_params["vel_y"],
                    cmd_params["vel_yaw"],
