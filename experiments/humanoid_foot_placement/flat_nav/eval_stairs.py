...
from sim2sim_stairs import (StairPlanGenerator, PlanFollowingGaitGenerator, LMJPolicy,
                            quat_rotate_inverse, pd_control, detect_failure_mode,
                            FOOT_AND_CALF_GEOMS)
...
class EvaluationResult:
    ...
    n_riser_contacts: int = 0        # riser contact events during the episode
...
class StairEvaluator:
    ...
    def run_trial(self, step_height: float, trial_num: int, max_time: float = 30.0, 
        ...
        planner_ctrl = PlanFollowingGaitGenerator(
            ...
            simulation_dt * control_decimation, max_vert=MAX_VERT, feet_dist=FEET_DIST, 
            target_yaw=np.deg2rad(orientation_yaw), tolerance=tolerance)
        ...
        try:
            while True:
                ...
                if self.check_fallen(d=d, current_height=current_world_height, min_rel_height=0.15):
                    fell = True
                    failure_mode = detect_failure_mode(m, d, fall_height=float('inf'),
                                                      current_world_height=current_world_height)
                    # time lookback
                    RISER_LOOKBACK = 1.5  
                ...
                # Track riser contacts: foot/calf geom touching a stair riser
                for ci in range(d.ncon):
                    ...
                    if in_foot_set and abs(c.frame[6]) < 0.3:
    ...
    def print_summary(self):
        ...
        for height in heights:
            ...
            # Foot placement stats (over all trials that had at least one placement)
            ...
            # Failure mode breakdown (only failed trials)
