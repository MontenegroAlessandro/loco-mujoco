...
@struct.dataclass
class FootPlacementTargetRewardState:
    # gait_process: float
    last_qvel: Union[np.ndarray, jax.Array]
    last_action: Union[np.ndarray, jax.Array]
    time_since_last_touchdown: Union[np.ndarray, jax.Array]
...
class FootPlacementTargetReward(Reward):
    """
    ...
    def __init__(self, env: Any, **kwargs):
        ...
        # Penalty for the stance foot slipping
        self._stance_slip_coeff = kwargs.get("stance_slip_coeff", 2.0)
        # Penalty for the swing foot moving after it has arrived at the target.
        self._stopping_vel_coeff = kwargs.get("stopping_vel_coeff", 2.5)
    ...
    def __call__(
            self, state: Union[np.ndarray, jnp.ndarray], 
            action: Union[np.ndarray, jnp.ndarray],
            next_state: Union[np.ndarray, jnp.ndarray], 
            absorbing: bool, 
            info: Dict[str, Any],
        ...
        pos_err_sq = backend.sum(backend.square(swing_pos[:2] - swing_target_pos[:2])) # just (x,y)
        dot_q = backend.clip(backend.sum(swing_target_orn * swing_orn), -1.0, 1.0)
        orn_err = 1.0 - backend.square(dot_q)
        swing_pos_reward = self._tracking_swing_pos_w * backend.exp(-self._tracking_swing_pos_sharp * pos_err_sq)
        swing_orn_reward = self._tracking_swing_orn_w * backend.exp(-self._tracking_swing_orn_sharp * orn_err)
        ...
        if backend == jnp:
            stance_foot_vel = jax.lax.select(
                stance_is_right, data.sensordata[self._right_foot_sensor_adr],
                data.sensordata[self._left_foot_sensor_adr]
                )
        else: 
            stance_foot_vel = data.sensordata[self._right_foot_sensor_adr] if stance_is_right else data.sensordata[self._left_foot_sensor_adr]
        stance_foot_on_ground = feet_on_ground[stance_foot_idx]
        stance_slip_penalty = backend.sum(backend.square(stance_foot_vel[:2])) * stance_foot_on_ground
        ...
        penalty_rewards = (
            ...
            + orientation_reward 
            + torque_reward 
            + torque_tiredness_reward 
            + energy_reward 
            + z_vel_reward 
            + roll_pitch_vel_reward 
            + joint_vel_reward 
            + acceleration_reward 
            + root_acceleration_reward 
            + action_rate_reward 
            + joint_position_limit_reward 
            + low_gains_reward 
            # + feet_slip_reward 
            + feet_yaw_diff_reward 
            + feet_yaw_mean_reward 
            + feet_roll_reward 
        ...
        # total_reward = survival_reward + tracking_reward + penalty_rewards
        total_reward = tracking_reward + penalty_rewards
