...
@struct.dataclass
class GoalFootPlacementFromVelocityState:
    """
    State for a goal that maps root velocity commands into foot placement targets,
    with internal randomization like GoalChangingRandomRootVelocity.
    """
    left_foot_target_pos: jax.Array
    left_foot_target_orn: jax.Array
    right_foot_target_pos: jax.Array
    right_foot_target_orn: jax.Array
    swing_foot_idx: int
    goal_height: float
    gait_frequency: float
    gait_process: float
    commanded_vel: jax.Array   # [vx, vy, yaw_rate]
    # bookkeeping
    gait_horizon: int
    gait_counter: int


class GoalFootPlacementFromVelocity(Goal, RootAndFootPlacementVisualizer):
    """
    High-level goal that periodically samples a root-velocity command and
    deterministically converts it into per-foot placement targets for the LL.
    """
    ...
    def __init__(self,
                 ...
                 left_foot_site_name: str,
                 right_foot_site_name: str,
                 # velocity command limits (m/s, rad/s)
                 max_x_vel: float = 1.2,
                 max_y_vel: float = 0.8,
                 max_yaw_vel: float = 1.0,
                 # resampling behavior for commanded velocities
                 resample_rate: float = 1.0 / 800.0,
    ...
    def init_state(self, env, key, model, data, backend) -> GoalFootPlacementFromVelocityState:
        return GoalFootPlacementFromVelocityState(
            ...
            swing_foot_idx=0,
            goal_height=self.goal_height,
            gait_frequency=self.gait_frequency_range[0],
            gait_process=0.0,
            commanded_vel=backend.zeros(3),
            gait_horizon=1,
            gait_counter=0
        )

    def _sample_command(self, key, backend):
        """Sample a commanded velocity and gait frequency (like GoalChangingRandomRootVelocity)."""
        if backend == np:
            hold = (np.random.rand() < self.still_proportion)
            if hold:
                vx = vy = wz = 0.0
                gf = self.gait_frequency_range[0]
            else:
                vx = np.random.uniform(-self.max_x_vel, self.max_x_vel)
                vy = np.random.uniform(-self.max_y_vel, self.max_y_vel)
                wz = np.random.uniform(-self.max_yaw_vel, self.max_yaw_vel)
                gf = np.random.uniform(self.gait_frequency_range[0], self.gait_frequency_range[1])
            return np.array([vx, vy, wz]), gf, key
        else:
            key, k0, k1 = jax.random.split(key, 3)
            hold = jax.random.uniform(k0) < self.still_proportion
            vel_min = jnp.array([-self.max_x_vel, -self.max_y_vel, -self.max_yaw_vel])
            vel_max = jnp.array([ self.max_x_vel,  self.max_y_vel,  self.max_yaw_vel])
            sampled_v = jax.random.uniform(k1, shape=(3,), minval=vel_min, maxval=vel_max)
            vxvywz = jnp.where(hold, jnp.zeros(3), sampled_v)
            gf = jax.lax.cond(
                hold,
                lambda _: self.gait_frequency_range[0],
                lambda _: jax.random.uniform(key, minval=self.gait_frequency_range[0],
                                             maxval=self.gait_frequency_range[1]),
                operand=None
            )
            return vxvywz, gf, key

    def reset_state(self, env, model, data, carry, backend):
        key = carry.key
        # sample commanded velocity first
        cmd_v, gf, key = self._sample_command(key, backend)
        ...
        if backend == np:
            ...
            gp0 = backend.where(near_zero, rand_gp, gp0)

        # proceed with state creation (same as before)
        state = GoalFootPlacementFromVelocityState(
            left_foot_target_pos=data.site_xpos[self._foot_site_id_left],
            left_foot_target_orn=jnp_R.from_matrix(
                data.site_xmat[self._foot_site_id_left].reshape(3, 3) if backend == jnp
                else np_R.from_matrix(data.site_xmat[self._foot_site_id_left].reshape(3, 3)).as_matrix()
            ).as_quat(scalar_first=True),
            right_foot_target_pos=data.site_xpos[self._foot_site_id_right],
            right_foot_target_orn=jnp_R.from_matrix(
                data.site_xmat[self._foot_site_id_right].reshape(3, 3) if backend == jnp
                else np_R.from_matrix(data.site_xmat[self._foot_site_id_right].reshape(3, 3)).as_matrix()
            ).as_quat(scalar_first=True),
            swing_foot_idx=0,
            goal_height=self.goal_height,
            gait_frequency=gf,
    ...
    def _maybe_resample_command(self, env, model, data, carry, backend):
        ...
        if backend == np:
            if np.random.rand() < self.resample_rate:
                cmd_v, gf, key = self._sample_command(key, backend)
                gp0, key = decide_gait_phase(cmd_v, key)
                state = state.replace(commanded_vel=cmd_v, gait_frequency=gf, gait_process=gp0)
        else:
            key, k = jax.random.split(key)
            carry = carry.replace(key=key)
            resample = (jax.random.uniform(k) < self.resample_rate)
            cmd_v, gf, key2 = self._sample_command(key, backend)
            gp0, key2 = decide_gait_phase(cmd_v, key2)

            def _resample_fn(s):
                return s.replace(commanded_vel=cmd_v, gait_frequency=gf, gait_process=gp0)

            state = jax.lax.cond(resample, _resample_fn, lambda s: s, operand=state)
            carry = carry.replace(key=key2)

        observation_states = carry.observation_states.replace(**{self.name: state})
        return carry.replace(observation_states=observation_states)


    def _compute_step_target_from_command(self, data, state, backend):
        """Create new swing-foot target from held command at a gait-side flip."""
        R = jnp_R if backend == jnp else np_R

        vx, vy, wz = state.commanded_vel
        # each step spans half a cycle
        step_time = 0.5 / jnp.clip(state.gait_frequency, 1.0e-8, 1.0e8)

        dx = jnp.clip(vx * step_time, -self.max_step_scale, self.max_step_scale)
        dy_nom = jnp.clip(vy * step_time, -self.max_step_scale, self.max_step_scale)
        dyaw = jnp.clip(wz * step_time, -self.yaw_step_clip, self.yaw_step_clip)

        # Determine stance/swing from *new* phase
        left_swing = (state.gait_process < 0.5)
        swing_idx = (~left_swing).astype(jnp.int32) if backend == jnp else int(not (state.gait_process < 0.5))
        stance_idx = 1 - swing_idx
        stance_is_right = (stance_idx == 1)

        stance_site = jax.lax.select(stance_is_right, self._foot_site_id_right, self._foot_site_id_left)
        swing_site  = jax.lax.select(~stance_is_right, self._foot_site_id_right, self._foot_site_id_left)

        stance_pos = data.site_xpos[stance_site]
        stance_mat = data.site_xmat[stance_site].reshape(3, 3)
        stance_orn = R.from_matrix(stance_mat).as_quat(scalar_first=True)

        # lateral sign convention: positive outward for each side
        side_sign = jnp.where(stance_is_right, 1.0, -1.0) if backend == jnp else (1.0 if stance_is_right else -1.0)
        step_vec_local = jnp.array([dx, dy_nom * side_sign, 0.0]) if backend == jnp else np.array([dx, dy_nom * side_sign, 0.0])

        target_pos = stance_pos + step_vec_local
        ...
        if backend == np:
            if swing_idx == 0:
                state = state.replace(
                    left_foot_target_pos=target_pos,
                    left_foot_target_orn=target_orn,
                    right_foot_target_pos=stance_pos,
                    right_foot_target_orn=stance_orn,
                    swing_foot_idx=swing_idx
                )
            else:
                state = state.replace(
                    right_foot_target_pos=target_pos,
                    right_foot_target_orn=target_orn,
                    left_foot_target_pos=stance_pos,
                    left_foot_target_orn=stance_orn,
                    swing_foot_idx=swing_idx
                )
        else:
            state = jax.lax.cond(
    ...
    def get_obs_and_update_state(self, env, model, data, carry, backend):
        """
        1) Maybe resample commanded velocities (rare).
        2) If the gait side flips, compute a NEW step target from the HELD command.
        3) Build LL observation (exactly like GoalDoubleFootPlacement).
        4) Advance gait phase.
        """
        # 1) maybe resample command
        carry = self._maybe_resample_command(env, model, data, carry, backend)
        state = getattr(carry.observation_states, self.name)

        # Detect side flip
        gp = state.gait_process
        left_swing = (gp < 0.5)
        swing_idx = (0 if not left_swing else 1) if backend == np else (~left_swing).astype(jnp.int32)
        resample_goal = (swing_idx != state.swing_foot_idx)

        # 2) on side flip -> recompute step target from HELD command
        if backend == np:
            if resample_goal:
                state = self._compute_step_target_from_command(data, state, backend)
        else:
            state = jax.lax.cond(
                resample_goal,
                lambda s: self._compute_step_target_from_command(data, s, backend),
                lambda s: s,
                operand=state
            )

        # 3) Build observation exactly like GoalDoubleFootPlacement.get_obs_and_update_state
        R = jnp_R if backend == jnp else np_R

        # Root rotation
        global_pose_root = data.qpos[self._root_qpos_ids]
        global_quat = global_pose_root[3:7]
        global_rot = R.from_quat(quat_scalarfirst2scalarlast(global_quat))

        # feet current world poses
        left_pos_w  = data.site_xpos[self._foot_site_id_left]
        left_mat_w  = data.site_xmat[self._foot_site_id_left].reshape(3, 3)
        left_quat_w = R.from_matrix(left_mat_w).as_quat(scalar_first=True)

        right_pos_w  = data.site_xpos[self._foot_site_id_right]
        right_mat_w  = data.site_xmat[self._foot_site_id_right].reshape(3, 3)
        right_quat_w = R.from_matrix(right_mat_w).as_quat(scalar_first=True)

        # target offsets (world)
        left_pos_offset_w  = state.left_foot_target_pos  - left_pos_w
        right_pos_offset_w = state.right_foot_target_pos - right_pos_w

        # transform to root-local
        left_pos_offset_local  = global_rot.apply(left_pos_offset_w, inverse=True)
        right_pos_offset_local = global_rot.apply(right_pos_offset_w, inverse=True)

        # orientation offsets (local, via foot frame)
        left_foot_matrix  = R.from_quat(quat_scalarfirst2scalarlast(left_quat_w))
        right_foot_matrix = R.from_quat(quat_scalarfirst2scalarlast(right_quat_w))

        left_R_target  = R.from_quat(quat_scalarfirst2scalarlast(state.left_foot_target_orn))
        right_R_target = R.from_quat(quat_scalarfirst2scalarlast(state.right_foot_target_orn))

        left_local_target_offset_orn  = (left_foot_matrix.inv()  * left_R_target).as_quat(scalar_first=True)
        right_local_target_offset_orn = (right_foot_matrix.inv() * right_R_target).as_quat(scalar_first=True)

        # hemisphere correction
        if backend == jnp:
            sgn = jnp.where(left_local_target_offset_orn[0] < 0, -1.0, 1.0)
            left_local_target_offset_orn = left_local_target_offset_orn * sgn
            sgn = jnp.where(right_local_target_offset_orn[0] < 0, -1.0, 1.0)
            right_local_target_offset_orn = right_local_target_offset_orn * sgn
        else:
            if left_local_target_offset_orn[0] < 0:  left_local_target_offset_orn  = -left_local_target_offset_orn
            if right_local_target_offset_orn[0] < 0: right_local_target_offset_orn = -right_local_target_offset_orn

        # observation layout (dim = 16)
        observation = backend.concatenate([
            left_pos_offset_local,
            left_local_target_offset_orn,
            right_pos_offset_local,
            right_local_target_offset_orn,
            backend.array([state.gait_process, state.gait_frequency]),
        ])

        # 4) advance gait phase
        gp_new = backend.fmod(state.gait_process + env.dt * state.gait_frequency, 1.0)
        state = state.replace(gait_process=gp_new)
        observation_states = carry.observation_states.replace(**{self.name: state})
        carry = carry.replace(observation_states=observation_states)
    ...
    @property
    def dim(self) -> int:
        # left pos(3)+orn(4) + right pos(3)+orn(4) + gait(2) = 16
        return 16
